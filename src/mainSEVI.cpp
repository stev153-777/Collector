    ...
    while (true) {
        ...
        if (do_execute_main_task) {
            ...
            switch (robot_state) {
                ...
                case RobotState::CHECKING_COLOR: {
                    ...
                    if (color_valid) {
                        ...
                        // Nur bei der ersten gueltigen Farbe:
                        // zuerst weiter per Line Follow, dann encoderbasiert rueckwaerts,
                        // danach normal weiter mit Line Follow. Kein Magazin-/Greifablauf.
                        if (!first_valid_crossline_done) {
                            first_valid_crossline_done = true;

                            state_timer.reset();
                            state_timer.start();

                            robot_state = RobotState::FIRST_LINE_FOLLOW_FORWARD;
                        }
                        else {
                            magazine_motor.setMaxVelocity(velocity_100);
                ...
                case RobotState::FIRST_LINE_FOLLOW_FORWARD: {
                    printf("FIRST_LINE_FOLLOW_FORWARD\n");

                    // Weiterfahren mit normalem Line-Follow-Control
                    motor_left.setVelocity(lineFollower.getLeftWheelVelocity());
                    motor_right.setVelocity(lineFollower.getRightWheelVelocity());

                    if (duration_cast<milliseconds>(state_timer.elapsed_time()).count() >= first_line_follow_forward_ms) {
                        robot_state = RobotState::FIRST_LINE_DRIVE_BACKWARD;
                    }
                    break;
                }

                case RobotState::FIRST_LINE_DRIVE_BACKWARD: {
                    printf("FIRST_LINE_DRIVE_BACKWARD\n");
                    ...
                    target_left_rotation = motor_left.getRotation() + first_line_backward_rot;
                    target_right_rotation = motor_right.getRotation() + first_line_backward_rot;

                    motor_left.setRotationRelative(first_line_backward_rot);
                    motor_right.setRotationRelative(first_line_backward_rot);

                    robot_state = RobotState::FIRST_LINE_WAIT_BACKWARD;
                    break;
                }

                case RobotState::FIRST_LINE_WAIT_BACKWARD: {
                    printf("FIRST_LINE_WAIT_BACKWARD\n");
                ...
                case RobotState::ARM_DOWN: {
                    ...
                    if (placing) {
                        target_rotation = -grip_offset_placing;
                        }
                    target_position_absolute = magazine_motor.getRotation() + target_rotation;
                    magazine_motor.setRotationRelative(target_rotation);
            ...
            if (do_reset_all_once) {
                ...
                target_left_rotation = 0.0f;
                target_right_rotation = 0.0f;
            }
        }
